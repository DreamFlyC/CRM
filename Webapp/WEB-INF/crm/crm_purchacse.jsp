...
<body>
	...
	<div class="rightContainer">
		...
		<button class="btn btn-primary"
			onclick="LW.location.forward('<%=basePath%>manage/crm/crm_order')">
		...
		<table fit="true" fitColumns="true"
			...
			<tbody>
				<c:choose>
					<c:when test="${crmPurchacseList.size() >0}">
						<c:forEach items="${pager.datas}" var="crmPurchacse">
							...
							<tr id="id${crmPurchacse.id}">
								...
								<td ><a href="manage/crm/crm_pccupdate?id=${crmPurchacse.id}">修改订单</a></td>
							</tr>
						</c:forEach>
					</c:when>
				</c:choose>
		...
		<c:if test="${crmPurchacseList.size()<=0||crmPurchacseList.size()==null}">
		...
		<nav>
			...
			<div class="btn-group pull-right" role="group">
				<a href="<%=basePath%>manage/crm/crm_find" class="btn btn-default">首页</a>
				<a href="<%=basePath%>manage/crm/crm_find?pageNo=${pager.prePage}"
					class="btn btn-default">上一页</a> <a
					href="<%=basePath%>manage/crm/crm_find?pageNo=${pager.nextPage}"
					class="btn btn-default">下一页</a> <a
					href="<%=basePath%>manage/crm/crm_find?pageNo=${pager.totalPage}"
	...
	<!--左侧导航栏智能显示  -->
		<script type="text/javascript">
			$("#page_crm_find").parent().attr("class","active");
		</script>
