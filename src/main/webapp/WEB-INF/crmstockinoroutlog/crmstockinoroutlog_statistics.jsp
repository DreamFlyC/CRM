...
<div class="place">
    ...
    <ul class="placeul">
        ...
        <li><a href="<%=basePath%>manage/crmstockinoroutlog/crmstockinoroutlog_list.html">进出货列表</a></li>
        <li>进出货统计</li>
    </ul>
</div>
...
<div class="cls"></div>
    <script>
        ...
        $(function () {
            $.ajax({
                url:"manage/crmstockinoroutlog/crmstockinoroutlog_getstatistics",
                type:"POST",
                dataType:"json",
                error:function(data){
                    console.log(data);
                ...
                success:function(data){
                    for(var i=0;i<data.msg.length;i++){
                        var name=data.msg[i].crmStockType.name;
                        type.push(name);
                        price.push(data.msg[i].price);
                        var msg={};
                        msg.value=data.msg[i].price;
                        msg.name=data.msg[i].crmStockType.name;
                        pieData.push(msg);
                    }
                    showchartBar();   //图表执行
                    showchartPie();
